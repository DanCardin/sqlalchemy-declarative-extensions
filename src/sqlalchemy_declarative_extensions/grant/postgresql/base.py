...
def for_role(role: Union[str, Role]):
    role_name = role.name if isinstance(role, Role) else role
    return GrantPrivileges(role_name)


@dataclass(frozen=True)
class GrantPrivileges(Generic[G]):
    target_role: str
    ...
    def grant(self, grant: str, *grants: str) -> GrantPrivileges:
        return replace(self, grants=tuple([grant, *grants]))

    def revoke(self, grant: str, *grants: str) -> GrantPrivileges:
    ...
    def on_table(self, *tables: str):
        return Grant(self, grant_type=GrantTypes.table, targets=tuple(tables))

    def in_schema(self, *schemas: Union[str, Schema]):
        schema_names = [s.name if isinstance(s, Schema) else s for s in schemas]
        return Grant(self, grant_type=GrantTypes.schema, targets=tuple(schema_names))
...
@dataclass(frozen=True)
class DefaultGrant(Generic[G]):
    privileges: GrantPrivileges[G]
