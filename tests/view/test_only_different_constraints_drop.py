...
view = View(
    ...
    materialized=True,
    constraints=[
        ViewIndex(["id"], unique=True),
        ViewIndex(["id"], unique=False, name="meow"),
    ],
)
...
def test_constraint_changes(pg):
    ...
    pg.execute(text('CREATE UNIQUE INDEX "ix_id" ON "fooschema"."bar" (id)'))
    pg.execute(text('CREATE UNIQUE INDEX "uq_id" ON "fooschema"."bar" (id)'))
    pg.commit()
    ...
    assert sql_statements[0] == """DROP INDEX "fooschema"."uq_id";"""
    assert sql_statements[1] == """CREATE INDEX "meow" ON "fooschema"."bar" (id);"""
