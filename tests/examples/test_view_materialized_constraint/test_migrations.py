...
def test_apply_autogenerated_revision(alembic_runner: MigrationContext, alembic_engine):
    ...
    with pytest.raises(sqlalchemy.exc.IntegrityError):
        alembic_engine.execute(text("REFRESH MATERIALIZED VIEW bar"))

    alembic_engine.execute(text("REFRESH MATERIALIZED VIEW baz"))
    result = list(alembic_engine.execute(text("select num, num2 from baz")).fetchall())
