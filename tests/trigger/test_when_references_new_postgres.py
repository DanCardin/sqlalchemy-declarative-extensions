"""Exercises parsing of the WHEN clause in a trigger definition.

When using `pg_get_expr` (historically) we got `bogus varno: 2`,
which breaks any trigger comparison attempts. Instead, we now
use regex to pull the field out (same way `\\d+ information_schema.triggers` does).
"""
